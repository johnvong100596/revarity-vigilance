...
- **ROUND-3 AUDIT done** (a11y, perf, onboarding/email). Caught + FIXED a CRITICAL self-inflicted regression: my revarity migration had dropped workspace-creation from handle_new_user → new signups hit a redirect loop. Fixed in prod (migration `20260529120006`, backfilled, verified 0 orphan profiles). Also fixed: fx_rates query bounded, snapshot composite index added.
  - FLAGGED for tomorrow (in AUDIT-POSTPUBLISH doc): a11y/design pass (contrast/type/tap-targets/hover-only content — the "anyone can use it" bar), perf optimizations (cron N+1, fx cache, select *), email deliverability (List-Unsubscribe + DMARC sender alignment), @revarity first-run explanation.
- **All migrations applied to prod (7 total tonight):** billing tables, RLS hardening, revarity comp, fx widen, snapshot index, signup-workspace fix, signup gen_random_bytes fix. tsc+lint+build green throughout.
- **🔴 SIGNUP WAS HARD-BROKEN — now fixed + PROVEN.** Investigating "does it really affect the app" surfaced that signup threw `function gen_random_bytes(integer) does not exist`: the trigger pins `search_path = public` but gen_random_bytes (pgcrypto) lives in `extensions`. Migrations masked it (the runner sees extensions; the trigger doesn't). Fix (`20260529120007`): generate tokens with core `gen_random_uuid()` instead. **Verified end-to-end** by creating real users (normal + @revarity.com) via the admin API → both get profile + workspace + active_workspace_id + tokens (+ operator/entity for revarity) → then deleted. Swept all migrations for the same bug class: handle_new_user was the only runtime offender; invite/workspace tokens in app code use JS randomBytes (safe).
- **PROCESS LESSON (important):** I had been applying migrations and asserting success without runtime-testing the actual user path. Two consecutive "fixes" were silently broken. Now using real create-user-then-delete smoke tests as the verification bar, not migration-apply success.
